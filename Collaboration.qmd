...
[**Finding 1: Individual cohorts are denser than the overall On Nigeria network.**]{style="color: #00457c;"}

The densities of individual cohort neighborhoods are higher than the overall network. For these purposes a cohort neighborhood includes those organizations that are in the cohort, and organizations that cohort members are directly connected to. The density of the overall network is `{r} round(density_non_end*100, digits = 0)` percent. This aligns with the density of the baseline study, and should be read as neither high nor low.

```{r, fig.align="center", fig.width=6, fig.height=4, out.width="75%", fig.cap="Figure 1. The densities of the overall networks and the cohorts compared."}

density_df <- data.frame(network = c("Behavior Change", "Criminal Justice", "JoinBodi", "Media and Journalism", "Overall", "Media and Journalism", "Behavior Change", "Criminal Justice", "JoinBodi", "Overall")
                                     , density = c(bechange_nbhd_density, crimjust_nbhd_density, joinbodi_nbhd_density, medjourn_nbhd_density, g_non_density, .07, .06, .04, .04, .01)
                         , group = c("endline", "endline", "endline", "endline", "endline", "baseline", "baseline", "baseline", "baseline", "baseline")) |> 
                                mutate(density = round(density*100, digits = 0))

subtitle <- "<span style = 'font-size:12pt; color:#00457C;'>**Endline**</span><span style = 'font-size:12pt;'> network and cohort densities compared with </span><span style = 'font-size:12pt; color:#ADD8E6;'>**Baseline**</span><span style = 'font-size:12pt;'> densities</span>"

ggplot(density_df)+
  geom_col(aes(x = density, y = fct_reorder(network, density), fill = group)
           #, fill = ""
           , width = .6
  ...
  geom_text(aes(x = density, y = fct_reorder(network, density)
            ...
            , fontface = "bold"
            , color = "white"
            , data = density_df %>% filter(group == "endline"))+
  geom_text(aes(x = density, y = fct_reorder(network, density)
                , label = paste0(density, "%"))
            , position = position_dodge(width = .8)
            , vjust = 1.5, size = 3
            , hjust = 1.25
            , fontface = "bold"
            , data = density_df %>% filter(group == "baseline"))+
  theme_void() +
  theme(axis.text.y = element_text(color = "black", size = 10, hjust = 1, face = "bold")
  ...
  labs(x = "Density"
       , title = "Network density for the overall network and individual \ncohort-level neighborhoods."
       , subtitle = subtitle) +
  scale_fill_manual(values = c("endline" = "#00457C", "baseline" = "#ADD8E6"))
...
This ensures that the full effect of cohorts as organizing bodies is captured in the findings. The criminal justice neighborhood has the highest density but is also the smallest neighborhood with only 31 organizations.[^collaboration-1] The quote below illustrates how cohorts are useful even for those outside of the formal cohort.

[^collaboration-1]: Behavior Change neighborhood has `{r} gorder(bechange_nbhd)` organizations; JoinBodi neighborhood has `{r} gorder(joinbodi_nbhd)` organizations; Media and Journalism has `{r} gorder(medjourn_nbhd)` organziations
...
```{r, fig.align="center", fig.width=12, fig.height=10, out.width="100%", fig.cap="Figure 2. The entire network displayed with cohorts by color and grantee by shape."}
...
![Figure 3: The baseline network showing data flows](data/baseline_network_vis1.png)
:::

[**Finding 2: Members of the On Nigeria-related portion of the accountability ecosystem tend to be highly collaborative.**]{style="color: #00457c;"}

Among all connections, respondents rated `r df_collab$percent_high` percent[^collaboration-2] of them as highly collaborative. This means that there is frequent communication, long-term interaction and coordination, and that the partners share ideas and resources. In particular, high collaboration was reported among non-grantee--grantee relationships where `r df_collab_grantee |> filter(from_grantee == "Non Grantee", to_grantee == "Grantee") |>  pull(percent_high)` percent[^collaboration-3] were considered high collaboration. Figure 4 shows a network map of levels of collaboration and Figure 5 shows levels of collaboration among grantees and non-grantees. Notably, only four percent of all collaborations were considered low. In the baseline only seven percent of collaborations were rated as Low as well. Low level collaboration refers to infrequent networking and/or infrequent information sharing.

[^collaboration-2]: `r df_collab$n_high` connections

[^collaboration-3]: `r df_collab_grantee |> filter(from_grantee == "Non Grantee", to_grantee == "Grantee") |>  pull(n_high)` connections

::: panel-tabset
## The Network highlighting collaboration level

```{r, fig.align="center", fig.width=12, fig.height=10, out.width="100%", fig.cap="Figure 4. The entire network displayed with cohorts by color and grantee by shape."}
...
E(g_non)$level_collab1 <- str_trim(factor(E(g_non)$level_collab1, levels = c("High", "Medium", "Low")))

#Main graph
ggraph(g_non, layout = "stress") +
  geom_edge_link0(aes(linetype = level_collab1), edge_color = "lightgrey", edge_linewidth = .8, alpha = .8
                  , arrow = arrow(angle = 15, length = unit(.05, "inches")
                                  , ends = "last", type = "closed")) +
  geom_node_point(aes(size = degree, shape = grantee), color = "white") +
  geom_node_point(aes(size = degree, shape = grantee, color = cohort)) +
  ...
  guides(color = "none", fill = "none"
         , shape = guide_legend(override.aes = list(size = 6))
         , linetype = guide_legend(title = "Connection\nLevel"))

```

## Levels of Collaboration

```{r, fig.cap="Figure 5: The majority of collaborations were rated as high, and this was especially true among non-grantee to grantee connetions."}
...
```{r, fig.cap="Figure 6: Intra cohort collaborations were almost always rated as either high or medium, meaning that these partners are engaging in longer term sharing and partnerships."}
...
[**Finding 3: There are a few central organizations across all cohorts that serve as hubs and play the role of key connectors across the network.**]{style="color: #00457c;"}
...
```{r, fig.align="center", fig.width=12, fig.height=10, out.width="100%", fig.cap="Figure 7. Hub organizations are located closer to the center. These hubs act as connectors across the whole network."}
...
[**Finding 4: The majority of grantees interact more with non-grantees, followed by grantees in their cohort, than with grantees in other cohorts. The exception is Media and Journalism grantees, who most commonly engage with fellow cohort members.**]{style="color: #00457c;"}
...
```{r, fig.align="center", fig.width=12, fig.height=6, out.width="100%", fig.cap="Figure 8. Cohort members are most frequently connected with non-grantees and next most connected with their own cohort grantees."}
...
ggplot(df_sankey, aes(axis1 = cohort.x
  ...
  labs(title = "Most collaborations are from cohorts to non-grantees"
       , x = "From cohort"
       , y = "To cohort") +
...
[**Finding 5: Network actors frequently support political participation and/or collective advocacy, and civil society oversight of government performance in their work to reduce corruption**]{style="color: #00457c;"}

Collaboration stories shared during FGDs demonstrate that both grantees and non-grantees primarily engage in vertical social accountability approaches. Of the the 34 collaboration stories shared, 22 used vertical approaches[^collaboration-4], 10 used horizontal approaches[^collaboration-5], and two used diagonal approaches[^collaboration-6].

[^collaboration-4]: When individual citizens, groups, and organizations play a direct role in holding governments to account using political voice through participation in democratic political processes, and with service providers using consumer voice. There are formal processes, such as elections, which are an institutional channel of vertical accountability; and there are informal processes through which citizens organize themselves into associations capable of lobbying governments and private service providers, demanding explanations and threatening fewer formal sanctions like negative publicity.

[^collaboration-5]: Consists of formal relationships within the state itself, with a focus on internal checks and oversight processes, whereby one state actor has the formal authority to demand explanations or impose penalties on another. For example, executive agencies must explain their decisions to legislatures, and can in some cases be overruled or sanctioned for procedural violations.

[^collaboration-6]: Refers to hybrid combinations of vertical and horizontal oversight, involving direct citizen engagement within state institutions. This can involve either participation in or direct management of official oversight bodies.
...
[**Finding 6: Organizations collaborate in many different ways. The most frequent way mentioned was learning, information, and knowledge sharing, but it varies depending on the types of organizations collaborating.**]{style="color: #00457c;"}
...
```{r, fig.cap="Figure 9. When identifying the ways that they are collaborating, learning, information, and knowledge sharing is the most frequent selection."}

flex_fun(data = ways_collab_grantee_ft)
...
[**Finding 7: Grantees' collaborative initiatives led to several quick wins. And in some cases, ongoing partnerships may have contributed to longer-term achievements.**]{style="color: #00457c;"}
