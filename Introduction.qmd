...
[^1]: These elements are contained in the Portfolio Rubrics of the Final Evaluation design.
...
The endline network is made up of different types of organizations. The most common type of organization is a non-governmental organiztion (NGO) which makes up `r ngo_orgs` percent[^2] of all the organizations. This represents a `r 63 - ngo_orgs` percent decrease in the percent of NGOs in the endline network compared to the baseline network. The next most frequent organization type are Ministries, Departments, and Agencies (MDAs), making up `r govt_orgs` percent[^3] which is consistent with the baseline network (24 percent). `r private_orgs` percent[^4] of actors in the network are private businesses, an increase of `r private_orgs-13` percent from the baseline. Finally, international NGOs (INGOs) make up `r ingo_orgs` percent[^5] of the endline network.

[^2]: `r length(dat5_non5$desc_org[dat5_non5$desc_org=="NGO"])` organizations

[^3]: `r length(dat5_non5$desc_org[dat5_non5$desc_org=="MDA"])` organizations

[^4]: `r length(dat5_non5$desc_org[dat5_non5$desc_org=="Private Business"])` organizations

[^5]: `r length(dat5_non5$desc_org[dat5_non5$desc_org=="INGO"])` organizations
